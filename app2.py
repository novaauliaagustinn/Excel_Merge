...
if uploaded_files:
    ...
    for i, file in enumerate(uploaded_files):
        ...
        # baca sebagai string
        excel_data = pd.read_excel(file, sheet_name=None, header=None, dtype=str)
    ...
    def to_excel(df):
        ...
        # Step 1: tulis file biasa
        writer = pd.ExcelWriter(output, engine='openpyxl')
        df.to_excel(writer, index=False, sheet_name='Merged Data')
        writer.close()

        # Step 2: buka workbook yang baru dibuat
        output.seek(0)
        wb = load_workbook(output)
        ws = wb.active

        # Step 3: HAPUS bold dari header
        for cell in ws[1]:
